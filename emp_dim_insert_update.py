...
def join_and_detect_new_or_changed_rows():
    ...
    emp_detail = pd.read_csv('s3://staging.emp.data/habiba_hr_sal.csv')
    emp_detail.drop(columns='Unnamed: 0', inplace=True)
    emp_sal = pd.read_csv('s3://staging.emp.data/habiba_emp_data.csv')
    ...
    conn = connect(
            user='HABIBA209',
            password='Habeba209aly',
            account='fo54415.eu-central-1'
        )
